...
export async function POST(
  ...
  try {
    ...
    const guidancePrompt = getGuidancePrompt(
      ...
      dispute.documentPlan?.documents.length || 0
    );

    // Add recent timeline context for more accurate responses
    let timelineContext = "";
    if (dispute.caseEvents.length > 0) {
      timelineContext = `\n\nRecent Timeline Events:\n`;
      timelineContext += dispute.caseEvents
        .map((event) => {
    ...
    return NextResponse.json(
      {
        response: aiContent,
        phase: dispute.phase,
        documentsCount: dispute.documentPlan?.documents.length || 0,
